...
fields = [
  { index_field_name: "abstract",    index_field_type:      "text",
                                     text_options:          { highlight_enabled: true } },
  { index_field_name: "authors",     index_field_type:      "text-array",
                                     text_array_options:    { highlight_enabled: true } },
  { index_field_name: "campuses",    index_field_type:      "literal-array",
                                     literal_array_options: { search_enabled: true,
                                                              facet_enabled: true } },
  { index_field_name: "content_types", index_field_type:    "literal-array",
                                     literal_array_options: { search_enabled: true,
                                                              facet_enabled: true } },
...
def fieldsEqual(f1, f2, path)
  if f1.nil?
    ...
    Set.new(f1.keys + f2.keys).each { |k|
      ret &&= fieldsEqual(f1[k], f2[k], "#{path}/#{k}")
    }
    return ret
  elsif f1 != f2
    puts "  #{path}=#{f2.inspect} (was #{f1.inspect})"
