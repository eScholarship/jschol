...
class SearchControls extends React.Component {
  render() {
    ...
    if (p.unitID) {
      searchUnit = [
          <input key="r2" type="radio" id="c-search2__refine-campus" name="searchType" value={p.unitID} onFocus={this.props.makeActive} onBlur={this.props.makeInactive}/>,
          <label key="l2" htmlFor="c-search_2_refine-campus">This {p.label}</label>
          ];
    }

    return (
    <div className={this.props.refineActive ? "c-search2__refine--active" : "c-search2__refine"}>
      <input key="r1" type="radio" id="c-search2__refine-eschol" name="searchType" value="eScholarship" defaultChecked={true} onFocus={this.props.makeActive} onBlur={this.props.makeInactive}/>
...
class SearchComp2 extends React.Component {
  ...
  handleRadioSelect = event => {
    console.log("radio select:", event.target.value)
